...
def build_command(
    ...
    cmd = [
        ...
        "--yolo",
        "--cwd", workdir,
        # Headless ``grok -p`` only. Never add these (or --cwd / --max-turns /
        # --disallowed-tools / --yolo / --output-format / --resume) to
        # ``grok agent serve`` (pinned CLI: unexpected argument, exit 2).
        "--no-subagents",
        "--no-plan",
    ]
    if model:
